...
void
HOF (gpointer callback_data, guint callback_action, GtkWidget *widget) {
    ...
    if ((hof = fopen ("/usr/local/share/NSB/HOF.txt", "r")) != NULL) {
        nohof = fread (&hoftext, sizeof hoftext, 1, hof);
        if (!ferror (hof))
            nohof = 0;
        else
    ...
    if (nohof) {
        gchar NoHOFAvailable[256] = "Hall of Fame data is not available", *msg[5];
        gint x;

        for (x = 0; x < 5; x++)
            msg[x] = NULL;

        strcpy (&work[0], "Hall of Fame data is not available.\n");
        Add2TextWindow (&work[0], 1);

        msg[0] = &NoHOFAvailable[0];
        outMessage (msg);
    ...
    gtk_window_set_title (GTK_WINDOW (window), "Hall of Fame");
    ...
    gtk_text_insert (GTK_TEXT (text), fixed_font, NULL, NULL, hoftext, strlen (&hoftext[0]));
    ...
    g_signal_connect (G_OBJECT (pbutton), "clicked", G_CALLBACK (PrintHOF), NULL);
...
void
PrintHOF (GtkWidget *widget, gpointer *pdata) {
    gchar Printing[256] = "Printing Hall of Fame ...", *msg[5];
    gint x;

    print (&hoftext[0]);

    strcpy (&work[0], "Print Hall of Fame info.\n");
