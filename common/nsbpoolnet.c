...
make_connection (char *service, int type, char *netaddress, int ind) {
    ...
    if (type == SOCK_STREAM) 
        port = atoport (service, "tcp", 0);
    if (type == SOCK_DGRAM)
        port = atoport (service, "udp", 0);
    if (port == -1) {
        if (syslog_ent == YES)
            syslog (LOG_INFO, "make_connection - invalid socket type: %s", service);
        return -1;
    }
    addr = atoaddr (netaddress);
    if (addr == NULL) {
        if (syslog_ent == YES)
            syslog (LOG_INFO, "make_connection - invalid network address: %s - error %d", netaddress, h_errno);
        return -1;
    }
